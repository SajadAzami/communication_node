...
def frontier_is_new(new_frontier,frontiers_list):
    for i in frontiers_list:
        if(math.sqrt((new_frontier[0]-i[0]) ** 2 + (new_frontier[1]-i[1]) ** 2)<1.5):
            return False;
    return True;
...
def send_goal(goal_x,goal_y):
    ...
    goal.target_pose.header.frame_id = "/"+name_space + "/map";
...
def main():
    ...
    for i in range (0,number_of_robots):
        ...
        other_robots_list.append(MyWrapper(list_index=i,robot_name_space="robot"+str(i)));
    create_service();
    move_base_tools();
