...
def master_request_handler(req):
    ...
    global new_command;
    new_command=True;
    print ("received new command from master");
    command_from_master=req;
    response_toMaster=Master_serviceResponse();
    response_toMaster.agent_state="command_confirmed";
    response_toMaster.agent_x=Odom_data.pose.pose.position.x;
    response_toMaster.agent_y=Odom_data.pose.pose.position.y;
    return response_toMaster;

def start_services():
     global agent_client;
     global agent_server;
     agent_server = rospy.Service("agent_server", Master_service, master_request_handler);
     rospy.wait_for_service("master_server");
     try:
        agent_client = rospy.ServiceProxy("master_server", Agent_service);
        print ("master of the server found");
...
class MyPoint():
    def __init__(self, xVector, yVector,inital_result=None):
        self.type = "Point";
        self.result = inital_result;
        self.x = xVector;
        self.y = yVector;
...
class ContactMaster(smach.State):
    def __init__(self):
        ...
        self.goal = None;
        self.response_fromMaster=None;
        self.initial=True

    def send_the_service(self,service_toMaster):
        global agent_client;
        try:
           self.response_fromMaster = agent_client(service_toMaster);
        except rospy.ServiceException, e:
            print ("sending cordinates of the victim to master failed");
            self.response_fromMaster="failed";
    ...
    def execute(self, userdata):
        ...
        a = "taher";
        service_toMaster=Agent_serviceRequest();
        rate = rospy.Rate(1);
        while self.initial:
            if(new_command==True):
                self.initial=False;
        ...
        if(new_command==True):
            self.response_fromMaster=command_from_master;
            new_command=False;
            ...
            service_toMaster.vic_state=0;
            service_toMaster.vic_x=userdata.CM_input.data.x;
            service_toMaster.vic_y=userdata.CM_input.data.y;
            ...
            service_toMaster.agent_x=Odom_data.pose.pose.position.x;
            service_toMaster.agent_y=Odom_data.pose.pose.position.y;
            service_toMaster.vic_state=1+victims[len(victims)-1].is_alive;
            service_toMaster.vic_x=victims[len(victims)-1].vic_x;
            service_toMaster.vic_y=victims[len(victims)-1].vic_y;
            self.send_the_service(service_toMaster);
            ...
            service_toMaster.agent_y=Odom_data.pose.pose.position.y;
            self.send_the_service(service_toMaster);
        if self.response_fromMaster == "failed":
            self.send_the_service(service_toMaster);

        if self.response_fromMaster!="failed":
            if self.response_fromMaster.command == "victim_confirmed":
            ...
            elif self.response_fromMaster.command=="explore":
                b=MyPoint(self.response_fromMaster.goal_x,self.response_fromMaster.goal_y,"exploration_center");
                exploration_boundry=self.response_fromMaster.blocks;
                userdata.CM_output=b;
                return "move_toGoal";
            elif self.response_fromMaster.command=="standby":
                b=MyPoint(self.response_fromMaster.goal_x,self.response_fromMaster.goal_y,"standby");
                userdata.CM_output=b;
                return "move_toGoal";
            elif self.response_fromMaster.command=="detect_victim":
                b=MyPoint(self.response_fromMaster.goal_x,self.response_fromMaster.goal_y,"detection_center");
                exploration_boundry=self.response_fromMaster.blocks;
                userdata.CM_output=b;
                return "move_toGoal";
            elif self.response_fromMaster.command=="save_victim":
                b=MyPoint(self.response_fromMaster.goal_x,self.response_fromMaster.goal_y,"standby");
                userdata.CM_output=b;
                return "move_toGoal";
        else:
            b=MyPoint(0,0,"exploration_center");
            exploration_boundry=[Point(20,20,0),Point(20,-20,0),Point(-20,-20,0),Point(-20,20,0)];
            userdata.CM_output=b;
            return "move_toGoal";
...
class VictimDetected(smach.State):
    ...
    def waiting(self):
        ...
        try:
           response_fromMaster = agent_client(service_toMaster);
        except rospy.ServiceException, e:
            print ("sending cordinates of the victim to master failed");
...
class Standby(smach.State):
    ...
    def execute(self, userdata):
        ...
        move_base_cancel_publisher(GoalID());
        rate = rospy.Rate(1.5);
        while not rospy.is_shutdown():
            if new_command==True:
                break;
...
class  VictimSearching(smach.State):
    ...
    def point_planner(self):
        ...
        while y_min <= y_max:
            x_min=k;
            while x_min < x_max:
                self.points.append(Point(x_min,y_min, 0.0));
                x_min +=5;
            self.points.append(Point(x_max,y_min, 0.0));
            y_min+=5;
            if y_min>y_max:
                y_min=y_max;

    def go_for_point(self):
            rospy.loginfo("sending goal inorder to find victim");
            # Simple Action Client
            global current_goal_status;
            global current_victim_status;
            global sac;
    ...
    def execute(self, userdata):
        ...
        if(userdata.VS_input=="new boundry"):
            self.point_planner();
        if (self.counter >= len(self.points)):
            userdata.VS_output="search completed";
