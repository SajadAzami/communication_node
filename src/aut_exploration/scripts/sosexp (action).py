...
def start_services():
     global agent_client;
     global agent_server;
     agent_server = actionlib.SimpleActionServer("agent_server", Master_toAgentAction, execute_cb=master_request_handler, auto_start = True);
     agent_client=actionlib.SimpleActionClient("master_server", Agent_toMasterAction);
     agent_client.wait_for_server();
...
class ContactMaster(smach.State):
    ...
    def send_the_service(self,service_toMaster):
        global agent_client;
        agent_client.send_goal(service_toMaster);
    ...
    def execute(self, userdata):
        ...
        a = "taher";
        service_toMaster=Agent_toMasterGoal();
        rate = rospy.Rate(1);
        while self.initial:
            if(new_command==True):
                break;
        ...
        if(self.initial):
            print (robot_name_space);
            print (" initial state of ours");
            self.initial=False;

        elif(type(userdata.CM_input) is str):
             print (robot_name_space+"klsdjf"+userdata.CM_input);
...
class  VictimSearching(smach.State):
    ...
    def point_planner(self):
        ...
        for i in exploration_boundry:
            ...
            if i.y > y_max:
                ...
                y_min=i.y;
        print (robot_name_space+str(y_min)+"--"+str(y_max)+"--"+str(x_min)+"--"+str(x_max))
        for iy in range(int(y_min),int(y_max),5):
                    for ix in range(int(x_min),int(x_max),5):
                        print(robot_name_space+"---"+str(ix)+"--"+str(iy));
                        self.points.append(Point(ix,iy,0.0));
        print (robot_name_space);
        print("point planner finished");
    ...
    def go_for_point(self):
            ...
            goal.target_pose.header.stamp = rospy.Time.now();
            print(robot_name_space);
            print("about to send the goal to find victim");
            ...
            sac.send_goal(goal);
            print(robot_name_space);
            print("just send the goal to find the victim");
            odom_temp = Odom_data;
            rate = rospy.Rate(3);  # 3hz
            i = 0;
            print(robot_name_space+"--"+str(self.counter));
            while not rospy.is_shutdown():
                if new_command==True:
                    print(robot_name_space+"new command");
                    return;
                i+=1;
                if odom_temp.pose.pose.position.x == Odom_data.pose.pose.position.x and odom_temp.pose.pose.position.y == Odom_data.pose.pose.position.y:
                    i += 2;
    ...
    def execute(self, userdata):
        ...
        if(userdata.VS_input=="comingback"):
                print(robot_name_space+" i came back baby");
        ...
        print(robot_name_space);
        print("we are about to go for points");
        self.go_for_point();
        print(robot_name_space);
        print("we just passed going for points");
        if new_command==True:
            new_command=False;
            if command_from_master.command=="explore":
                b=MyPoint(command_from_master.goal_x,command_from_master.goal_y,"exploration_center");
